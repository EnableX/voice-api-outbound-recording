...
const {
  bridgeCall, makeOutboundCall, hangupCall,startRecording, stopRecording,
} = require('./voiceapi');
...
function  recordingStop() {
    logger.info(`[${callVoiceId}] stop the recording`);
    stopRecording(callVoiceId, () => {});
}
...
function voiceEventHandler(voiceEvent) {
  if (voiceEvent.state) {
    if (voiceEvent.state === 'connected') {
      ...
      logger.info(`[${callVoiceId}] ${eventMsg}`);
      sseMsg.push(eventMsg);
	  startRecording(callVoiceId , 'anand', () => {});
